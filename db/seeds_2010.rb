...
EventLoader.with_options(:day => 29) do |e29|
  e29.with_options(:room => big) do |e29_big|
    ...
    e29_big.create :title => 'Distributed storage system with ruby', :title_ja => 'Rubyによる分散ストレージシステムの実装', :speaker => 'Toshiyuki Terashita (RICOH IT SOLUTIONS Co.,Ltd.)', :abstract => "Castor is a new scalable storage system implemented in Ruby. In this session, we talk about the reasons why we decided to develop new Castor storage system, and the features against another storage systems such as MogileFS and kumoFS.", :abstract_ja => "Castorは、Rubyで実装されたスケーラブルなストレージシステムです。本発表では、開発に至った経緯と、MogileFSやkumoFS等、他の分散ストレージシステムとの違いについて紹介します。"
    e29_big.create :title => 'The last decade of RWiki and lazy me', :title_ja => 'RWikiと怠惰な私の10年間', :speaker => 'Masatoshi SEKI (druby.org)', :abstract => "#TODO", :abstract_ja => "2000-8-31、ruby-listにすこし変わったWikiが紹介されました。[ruby-list:24661] dRubyの最初の実用的なアプリケーション、RWikiです。
  ...
  e29.with_options(:room => medium) do |e29_medium|
    e29_medium.create :title => 'The future of the bigdecimal library and the number system of Ruby', :speaker => 'Kenta Murata (GeneticLab Co., Ltd. && Ruby Sapporo)', :from => '9:30'
    ...
    e29_medium.create :title => "How to survive in after Rails' time", :title_ja => 'before Rails 時代のプログラマが如何にして after Rails の世界にたどりついたか', :speaker => 'SHIBATA Hiroshi (Eiwa System Management,Inc. / asakusa.rb / tDiary.org)', :abstract => "In Before Rails' time, many programmers had used Ruby for developing products or useful tools. In After Rails' time, we need to change old thinking. I'll talk about my experience of changing my thinking to adapt to new days.", :abstract_ja => "before Rails 時代から私たちは多くのプロダクトや社内ツールをRubyによって作ってきました。しかし、After Rails の時代におけるアプリケーション開発では、before Rails 時代とは異なる考え方を持つ必要があります。このセッションでは、自分の経験した after Rails 時代に適応するための頭の切り替え方を話します。"
