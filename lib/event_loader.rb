class EventLoader
  ...
  def self.create(values)
    title, title_ja, abstract, abstract_ja, speaker, room, day, is_break, length, lang, parent_event = values[:title], values[:title_ja], values[:abstract], values[:abstract_ja], values[:speaker], values[:room], values[:day], (values[:break] || false), (values[:length] || 30.minutes), (values[:lang] || 'en'), values[:parent_event]
    date = "2010/8/#{day}"
    ...
    event = Event.create :presenter_name => speaker, :title_en => title, :title_ja => title_ja || title, :abstract_en => abstract, :abstract_ja => abstract_ja || abstract, :additional_info => 'あとでかく', :lang => lang, :break => is_break, :parent_event => parent_event
