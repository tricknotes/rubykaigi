class EventLoader
  ...
  def self.create(values)
    title, speaker, room, day, is_break, length = values[:title], values[:speaker], values[:room], values[:day], (values[:break] || false), (values[:length] || 30.minutes)
    date = "2010/8/#{day}"
    ...
    event = Event.create :title_en => title, :title_ja => title, :additional_info => 'あとでかく', :lang => 'ja', :break => is_break
    if speaker
